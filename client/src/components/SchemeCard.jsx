import React from 'react';
import { Sparkles, CheckCircle, Clock, ArrowRight, AlertCircle, ShieldAlert, ShieldCheck } from 'lucide-react';

const SchemeCard = ({ scheme, onApply }) => {
  const matchPercentage = scheme.matchPercentage || 85;
  const isEligible = scheme.isEligible !== undefined ? scheme.isEligible : (matchPercentage >= 50);
  const status = scheme.eligibilityStatus || (isEligible ? "Eligible" : "Not Eligible");

  return (
    <div className={`glass-panel glass-card-hover rounded-2xl p-6 relative flex flex-col justify-between overflow-hidden group border ${
      !isEligible ? 'border-rose-900/40 bg-gray-950/80 opacity-90' : 'border-gray-800'
    }`}>
      
      {/* Top Banner Accent */}
      <div className={`absolute top-0 left-0 right-0 h-1 ${
        isEligible 
          ? (status === "Eligible" ? 'bg-gradient-to-r from-emerald-400 via-teal-400 to-sky-400' : 'bg-gradient-to-r from-amber-400 to-orange-400')
          : 'bg-gradient-to-r from-rose-500 to-red-600'
      }`}></div>

      <div>
        {/* Header Tags & Status Badge */}
        <div className="flex items-center justify-between gap-2 mb-4">
          <span className="px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider bg-gray-900 border border-gray-800 text-emerald-400">
            {scheme.category || 'Welfare'}
          </span>

          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold font-mono border ${
            isEligible 
              ? (status === "Eligible" 
                  ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30 glow-emerald' 
                  : 'bg-amber-500/10 text-amber-400 border-amber-500/30')
              : 'bg-rose-500/10 text-rose-400 border-rose-500/30'
          }`}>
            {isEligible ? <ShieldCheck className="w-3.5 h-3.5" /> : <ShieldAlert className="w-3.5 h-3.5" />}
            {status} ({matchPercentage}%)
          </div>
        </div>

        {/* Title */}
        <h3 className="text-xl font-bold text-white group-hover:text-emerald-400 transition-colors mb-2 line-clamp-2">
          {scheme.schemeName}
        </h3>

        {/* Description */}
        <p className="text-sm text-gray-300 line-clamp-3 mb-4 leading-relaxed">
          {scheme.description}
        </p>

        {/* Benefits Highlight */}
        <div className="p-3 rounded-xl bg-gray-900/90 border border-gray-800/80 mb-4">
          <span className="text-xs font-semibold uppercase text-emerald-400 tracking-wider block mb-1">
            Benefits Provided:
          </span>
          <p className="text-xs text-gray-200 font-medium leading-normal">
            {scheme.benefits}
          </p>
        </div>

        {/* Matched AI Reasons */}
        {scheme.matchedReasons && scheme.matchedReasons.length > 0 && isEligible && (
          <div className="space-y-1 mb-4">
            {scheme.matchedReasons.slice(0, 2).map((reason, idx) => (
              <div key={idx} className="flex items-start gap-1.5 text-xs text-emerald-400/90">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                <span>{reason}</span>
              </div>
            ))}
          </div>
        )}

        {/* Unmatched Reasons for Ineligible Schemes */}
        {scheme.unmatchedReasons && scheme.unmatchedReasons.length > 0 && !isEligible && (
          <div className="space-y-1 mb-4">
            {scheme.unmatchedReasons.slice(0, 2).map((reason, idx) => (
              <div key={idx} className="flex items-start gap-1.5 text-xs text-rose-400/90">
                <AlertCircle className="w-3.5 h-3.5 text-rose-400 shrink-0 mt-0.5" />
                <span>{reason}</span>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* Footer Info & Action */}
      <div className="pt-4 border-t border-gray-800/80 flex items-center justify-between text-xs text-gray-400">
        <div className="flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5 text-gray-500" />
          <span>Deadline: {scheme.deadline || 'Open Year-round'}</span>
        </div>

        <button
          onClick={() => onApply && onApply(scheme)}
          disabled={!isEligible}
          className={`px-4 py-2 rounded-lg font-bold flex items-center gap-1.5 transition-all shadow-md ${
            isEligible 
              ? 'bg-emerald-500 hover:bg-emerald-400 text-gray-950 hover:shadow-emerald-500/20' 
              : 'bg-gray-800 text-gray-500 cursor-not-allowed'
          }`}
        >
          {isEligible ? 'Apply Now' : 'Ineligible'}
          {isEligible && <ArrowRight className="w-3.5 h-3.5" />}
        </button>
      </div>
    </div>
  );
};

export default SchemeCard;
